...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setInterval(loadDashboard, 600000); // ✅ reduced load (10 min)
...
function socketEvents(){
    ...
    socket.on("dashboard_update",(data)=>{
        ...
        // ✅ HANDLE PENDING SYMBOL
        if(pendingSymbol){
            select.value = pendingSymbol;
            pendingSymbol = null;
            loadDashboard();
        }
...
function loadDashboard(){
    ...
    if(!socket || !select || isRequestPending) return;
    ...
    requestTimeout = setTimeout(() => {
        console.log("⚠️ Request timeout reset");
        isRequestPending = false;
    }, 10000);
